...
(function() {
  var Asteroid, Button, Drawable, Engine, Hud, Image, Screen, Text, Vessel, canvas, context, createAsteroidStore, createSurface, debug, directionOf, distanceBetween, enableLog, game, inDocumentConsole, keymap, log, oldLog, showconsole, speedOf, surface, translate, willCollide,
    __slice = [].slice,
  ...
  Engine = (function() {
    ...
    Engine.prototype.updateCollisions = function(vessel, asteroids) {
      var alreadyExists, asteroid, asteroidGap, collision, current, distance, firstpass, gap, id, newCollisions, secondAsteroid, source, sourceAngle, sourceRotatedSpeed, sourceSpeed, target, targetAngle, targetRotatedSpeed, targetSpeed, vesselGap, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _m, _n, _ref, _ref1, _ref2;
      firstpass = [];
      ...
      newCollisions = [];
      for (_l = 0, _len3 = firstpass.length; _l < _len3; _l++) {
        current = firstpass[_l];
        alreadyExists = false;
      ...
      for (_n = 0, _len5 = newCollisions.length; _n < _len5; _n++) {
        collision = newCollisions[_n];
        if (!(!collision.target.collides && !collision.source.collides)) {
          continue;
        }
